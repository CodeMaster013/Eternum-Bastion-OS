...
CREATE POLICY "Users can view souls based on access level" ON souls
  FOR SELECT USING (
    CASE 
      ...
      WHEN access_level = 'restricted' THEN (auth.jwt() ->> 'user_metadata')::jsonb ->> 'access_level' IN ('executor', 'root')
...
CREATE POLICY "Executors and above can insert souls" ON souls
  FOR INSERT WITH CHECK (
    (auth.jwt() ->> 'user_metadata')::jsonb ->> 'access_level' IN ('executor', 'root')
  );
