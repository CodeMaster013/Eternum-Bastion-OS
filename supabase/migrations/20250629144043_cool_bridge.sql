/*
  ...
  1. New Tables
    - `souls` - Registry of all entities in the bastion with transformation tracking
    - `transformation_records` - History of all transformations performed
    - `spells` - Crafted spells and rituals with rune combinations
    - `prophecies` - Mystical predictions and visions
    - `system_events` - Log of all system activities and events
    - `energy_allocations` - Chamber power management and allocation

  2. Security
    - Enable RLS on all tables
    - Role-based access policies (guest, executor, root)
  ...
  3. Features
    ...
    - Default energy allocations for chambers
*/
...
BEGIN
  ...
  -- Drop transformation_records policies
  ...
  -- Drop energy_allocations policies
...
CREATE POLICY "Users can view souls based on access level" ON souls
  FOR SELECT USING (
    CASE 
      ...
      WHEN access_level = 'restricted' THEN ((auth.jwt() ->> 'user_metadata')::jsonb ->> 'access_level') = ANY (ARRAY['executor', 'root'])
      WHEN access_level = 'classified' THEN ((auth.jwt() ->> 'user_metadata')::jsonb ->> 'access_level') = 'root'
      ELSE false
    END
  );
...
CREATE POLICY "Executors and above can insert souls" ON souls
  FOR INSERT WITH CHECK (
    ((auth.jwt() ->> 'user_metadata')::jsonb ->> 'access_level') = ANY (ARRAY['executor', 'root'])
  );
...
CREATE POLICY "Only root can delete souls" ON souls
  FOR DELETE USING (
    ((auth.jwt() ->> 'user_metadata')::jsonb ->> 'access_level') = 'root'
  );
